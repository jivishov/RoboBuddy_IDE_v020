...
class FakeBackend {
  ...
  async loadScene(scene, context) {
    ...
    this.loaded = true;
    return { sceneRevision: scene.revision, robotId: scene.robotId, observation: { simulationTimeSeconds: 0 } };
  }
...
const loaded = await session.loadScene(structuredClone(PHASE1_SCENE));
...
assert.equal(fake.calls.at(-1)[1].modelPackage, PHASE1_SCENE.modelPackage);
assert.equal(fake.calls.at(-1)[1].physics.timestepSeconds, 0.002);
assert.equal(fake.calls.at(-1)[1].physics.integrator, 'RK4');
...
assert.equal(envelope.sceneRevision, PHASE1_SCENE.revision);
assert.equal(envelope.robotId, PHASE1_SCENE.robotId);
...
for (const token of ['mj_name2id', 'jnt_qposadr', 'jnt_dofadr', 'actuator_trnid', 'data.xpos', "crypto.subtle.digest('SHA-256'"]) {
  assert.ok(workerSource.includes(token), `worker must use named/address/model-identity access: ${token}`);
}
...
assert.ok(workerSource.includes('collection.delete?.()'), 'worker must release the copied MuJoCo contact vector');
assert.ok(workerSource.includes('contact.delete?.()'), 'worker must release copied MuJoCo contact handles');
...
assert.ok(backendSource.includes('#assertPhase1Scene'), 'backend must validate the pinned Phase 1 scene contract');
