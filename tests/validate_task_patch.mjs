...
if (visibleOpenarm.length !== 2 || visibleOpenarm[0].id !== 'openarm-04-filtration-workcell' || visibleOpenarm[1].id !== OPENARM_LAB_BUILDER_TASK.id || visibleOpenarm.some(item => item.simulationMode !== 'physical_mujoco')) throw new Error(`OpenArm physical catalog drift: ${JSON.stringify(visibleOpenarm)}`);
...
if (openarmScenario.workspaceRevision !== 'openarm-v2-physical-bimanual-stack-v3') throw new Error('OpenArm refined workspace revision drifted');
...
const labBuilderScenario = await loadPatchedScenario('openarm', OPENARM_LAB_BUILDER_TASK.id);
if (!labBuilderScenario?.labBuilder || labBuilderScenario.physicalSceneId !== 'openarm-lab-builder-empty-v1') throw new Error('OpenArm Lab Builder scenario did not resolve independently');
if (labBuilderScenario.portablePython.referenceActions.length !== 0) throw new Error('Lab Builder must not preload a reference task program');
const labBuilderWorkspace = buildPatchedWorkspace('openarm', labBuilderScenario);
for (const file of ['main.py','lab_project.py','robot_config.py','workcell.py']) if (!labBuilderWorkspace[file]) throw new Error(`Lab Builder starter missing ${file}`);
for (const forbidden of ['flask', 'beaker', 'hotplate', 'ring_gauze', '.grasp(', '.attach(', '.teleport(', '.move_to(']) if (labBuilderWorkspace['main.py'].includes(forbidden)) throw new Error(`Lab Builder starter leaks reference-task behavior ${forbidden}`);
if (!labBuilderWorkspace['workcell.py'].includes('preloaded_lab_assets') || !labBuilderWorkspace['workcell.py'].includes('[]')) throw new Error('Lab Builder starter must declare an empty authored-lab start');
...
for (const token of ['OpenArmPhysicalSimulator', 'OpenArmLabBuilderSimulator', "physical && profileId === 'openarm'", 'scenario?.labBuilder === true', 'So101PhysicalSimulator', 'LeKiwiPhysicalSimulator', "physical && profileId === 'lekiwi'"]) if (!host.includes(token)) throw new Error(`simulator host missing physical routing token ${token}`);
...
console.log('pinned legacy fixtures + SO-101/OpenArm reference/Lab Builder physical catalogs/starters: OK');
