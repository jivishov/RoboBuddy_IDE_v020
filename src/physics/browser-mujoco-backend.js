...
export class BrowserMuJoCoBackend {
  ...
  async acceptCommand(envelope) {
    ...
    this.#assertGeneration(generation, envelope); const observation = this.#decorateObservation(raw); this.lastObservation = observation;
  ...
  async advanceSteps(stepCount = 1, context = {}) {
    ...
    try {
      ...
      return observation;
    } catch (error) { if (generation === this.generation && this.loaded) this.state = PhysicsBackendState.FAILED; throw error; }
  }
  ...
  async getObservation(context = {}) {
    ...
    const generation = this.generation; const raw = await this.#call('observe'); this.#assertGeneration(generation, context);
  ...
  async pause(context = {}) { this.#assertLoaded(); this.#assertContext(context); const generation = this.generation; const raw = await this.#call('pause'); this.#assertGeneration(generation, context); const observation = this.#decorateObservation(raw); this.lastObservation = observation; this.state = PhysicsBackendState.PAUSED; this.#record('pause', { observation }); return observation; }
  async resume(context = {}) { this.#assertLoaded(); this.#assertContext(context); const generation = this.generation; const raw = await this.#call('resume'); this.#assertGeneration(generation, context); const observation = this.#decorateObservation(raw); this.lastObservation = observation; this.state = PhysicsBackendState.READY; this.#record('resume', { observation }); return observation; }
  ...
  dispose() { if (this.disposed) return; this.disposed = true; ++this.generation; this.#terminate('disposed'); this.loaded = false; this.state = PhysicsBackendState.DISPOSED; this.lastObservation = null; this.commandBudget = null; }
