...
export const ASIMOV_WORKSPACES=Object.freeze(Object.fromEntries(Object.entries(ASIMOV_SCENES).map(([variant,s])=>[s.id,Object.freeze({
  ...
  capabilities:{standing:variant==='standing'?'experimental-flat-floor-trial':'unsupported',actuatorProfile:ASIMOV_ALL_PACKAGES[variant].actuatorProfileId??'reference-ideal',sensorView:ASIMOV_ALL_PACKAGES[variant].sensorProfileId??'ground_truth',jointTargets:'supported',gravity:'supported',contacts:'source-primitives',walking:'unsupported',grasping:'unsupported',balanceRecovery:'unsupported',neck:'fixed in source'},
  taskEvaluation:{type:variant==='standing'?'bounded-standing-trial':'observation-only',syntheticSuccessEvents:false},limitations:ASIMOV_ALL_PACKAGES[variant].limitations,
})])));
// Mounted lab is the useful beginner default; free-base and passive drop remain first-class scenes.
export const ASIMOV_TASKS=Object.freeze(['mounted','freebase','drop','actuator-mounted','actuator-freebase','standing'].map(v=>ASIMOV_WORKSPACES[`asimov-${v}`]));
export function asimovWorkspaceFiles(scenario) {
  const passive=scenario.id==='asimov-drop';
  const standing=scenario.id==='asimov-standing';
