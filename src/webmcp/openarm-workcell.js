...
const projectSchema = { type:'object', properties:{ schema_version:{const:OPENARM_LAB_PROJECT_VERSION}, scene:sceneSchema, task:taskSchema, program:base.createOpenArmProgramSchema(), execution_profile:{type:'object'} }, required:['schema_version','scene'], additionalProperties:false };
...
export function getOpenArmWorkcellDefinitions(facade){
  ...
  return[
    {name:'inspect_openarm_workcell',title:'Inspect OpenArm Lab Builder',description:'Read the active Lab Builder SceneSpec/TaskSpec, assumptions, physical asset poses, contacts, pinch frames, scene revision, staged candidate and evaluator state. Simulator ground truth is labelled as such; image pixels are not required when an external multimodal agent supplied structured interpretation.',inputSchema:{type:'object',properties:{},additionalProperties:false},readOnly:true},
    {name:'manage_openarm_workcell',title:'Author OpenArm lab from structured image interpretation',description:`Use ${OPENARM_LAB_BUILDER_TOOL_VERSION}. stage_scene accepts an inspectable ${OPENARM_LAB_SCENE_VERSION} with reference provenance, quantity evidence, bench/equipment/obstacles and support relations. stage_project reopens an exported project but never starts its program. apply is transactional and explicitly resets to a new scene revision; failed compilation/overlap checks leave the active scene intact. set_task freezes the supported dry_transfer evaluator. plan_transfer creates an editable bounded program without changing the authored scene. export_project returns scene/task/program together. No arbitrary XML, code, URLs, evaluator code or physical-state setters.`,inputSchema:createOpenArmWorkcellSchema(),readOnly:false},
    {name:'run_openarm_program',title:'Run bounded OpenArm Lab Builder program',description:`Execute ${base.OPENARM_PROGRAM_VERSION} against the CURRENT approved Lab Builder revision. Programs are persisted in the project before execution. Optional feedback predicates use actual MuJoCo observations; application-owned task success remains independent of the user program. Human Stop or Agent Assist revocation cancels execution.`,inputSchema:base.createOpenArmProgramSchema(),readOnly:false},
  ];
}
...
export async function manageOpenArmWorkcell(facade,input,signal,epoch){
  ...
  try{
    ...
    else if(input.command==='plan_transfer')result=backend.planLabTransfer();
