...
Each object's `quantity_evidence` separates `geometry`, `pose`, `mass` and `friction`. Sources are `source_provided`, `user_measured`, `image_estimated`, `fitted` and `assumed`. Use `uncertainty_m` for geometric/pose uncertainty, and `relative_uncertainty` where appropriate for physical quantities. Missing evidence is explicitly assumed. User measurements remain declarations until compared with independent evidence.
...
`manage_openarm_task` needs that task, the current scene revision and `command:"assess"` or `"define"`. Object IDs are the SceneSpec IDs, without `lab_`. Insertion additionally identifies `object_port` and a minimum `insertion_depth_m`. Nominal bore clearance is compared with declared dimensional uncertainty. The assessment does not prove unobstructed access, other-part clearance, reachability, load capacity, graspability, collision-free motion or real-world fit.

Defining a task freezes its goal and rejects an already-held object or an object already in the receiving geometry. The independent evaluator needs initial released support, at least 60 ms of bilateral finger contact without palm contact, at least 20 mm lift without other support, at least 20 mm subsequent held displacement, release from both grippers, actual gripper retreat and at least 200 ms of settled receiving contact. Unreadable contacts, excessive penetration, non-monotonic/insufficient observation sampling, resets and scene changes invalidate the trial. Grasp/lift/carry/release/retreat milestones include actual observed body poses. Failure diagnostics expose a bounded list of offending contacts.
