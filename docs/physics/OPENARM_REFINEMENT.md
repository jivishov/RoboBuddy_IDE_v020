...
3. `discard` removes a preview. Staging/applying an empty list removes all custom equipment; baseline fixtures and vessels remain.

Each item has an identifier, kind and `position_m` describing **bottom-centre**. `dimensions_m` contains full X width, Y width and Z height, not half-sizes. Optional yaw is in radians about world Z. Supported kinds: platform, tray, rack, vial, block, button and assembly. Assemblies accept at most 12 box/sphere/cylinder parts with local positions and bounded dimensions. Trays/racks use separated components so cavities are not solid convex hulls. Vials are solid dry exterior models.

Limits: 12 equipment items, 128 compiled geometries, 24 kB equipment input, 0.005–2 kg dynamic item mass, and a bounded table workspace. Raw XML, arbitrary scripts, remote asset URLs and physics disabling are rejected. The scene compiler revalidates on the worker side.
