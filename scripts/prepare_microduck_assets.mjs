...
function classify(path) {
  ...
  // The Phase 5C controller-conformance fixture is generated by the native reference runner
  // from the pinned deployed controller sources, independently of the browser implementation.
  // It is the reference the browser controller is checked against, so it is classified apart
  // from the Cycle 01 inference-parity fixture.
  if (rel === 'fixtures/controller-conformance.json') return { license: 'PolyForm-Noncommercial-1.0.0', repository: 'local', revision: 'phase5c', sourcePath: 'native/microduck_reference.py --fixture', status: 'recorded_generated_evidence', purpose: 'independently derived MicroDuck controller conformance reference: 61-value observation assembly, raw policy output, action scaling and target filtering at sampled states' };
